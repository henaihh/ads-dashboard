...
export async function GET() {
  ...
  try {
    ...
    if (advertisers.length === 0) {
      return NextResponse.json({ error: 'No PADS advertisers found', raw: advData }, { status: 400 });
    }
    const advertiser = advertisers[0]; // Use first (likely MLA for Argentina)
    const advertiserId = advertiser.advertiser_id;

    // 2. Get campaigns with metrics (last 30 days)
    ...
    return NextResponse.json({
      ...
      totalCampaigns: campsData.paging?.total || campaigns.length,
    });
  } catch (err) {
